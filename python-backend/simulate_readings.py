...
BACKEND_URL = "http://10.203.142.134:8000"

async def check_system_state(session):
    """Ask the backend if the system is currently STARTED"""
    try:
        async with session.get(f"{BACKEND_URL}/api/system/state") as resp:
            if resp.status == 200:
                data = await resp.json()
                return data.get("active", False)
    except Exception as e:
        print(f"✗ Connection Error: Could not reach backend to check state: {e}")
    return False
    ...
    async with aiohttp.ClientSession() as session:
        try:
            while True:
                ...
                is_active = await check_system_state(session)
                ...
                if is_active:
                    ...
                    print(f"💤 [{datetime.now().strftime('%H:%M:%S')}] System is STOPPED. Waiting...")
