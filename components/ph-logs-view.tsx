"use client"
//sammeee code for the ph values that i used from temperature logs view 
...
export function PhLogsView() {
  const { phLogs, clearLogs } = useDashboard()

  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground text-balance">pH Logs</h1>
          <p className="text-sm text-muted-foreground">
            pH readings captured on every change during Forward phase
          </p>
        </div>
        <Button
          onClick={clearLogs}
        ...
        <CardContent className="p-0">
          {phLogs.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-3 py-16 text-muted-foreground">
              <Droplets className="size-8 opacity-40" />
